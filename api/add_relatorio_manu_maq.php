...
// O 'name' do campo no seu HTML é 'maquina', não 'codigo'
$codigo_maquina = $_POST['maquina']; 
$data_manutencao = $_POST['data'];
$observacoes = $_POST['observacoes'];
$id_usuario = $_SESSION['id']; // Pega o ID do usuário logado (o Funcionário)

// Validação simples
if (empty($codigo_maquina) || empty($data_manutencao)) {
    http_response_code(400); // Bad Request
    echo json_encode(['success' => false, 'error' => 'Codigo da maquina e Data sao obrigatorios']);
    exit();
}

$stmt = $conn->prepare("INSERT INTO relatorios_manutencao_maq (id_usuario, codigo_maquina, data_manutencao, observacoes) VALUES (?, ?, ?, ?)");
$stmt->bind_param("isss", $id_usuario, $codigo_maquina, $data_manutencao, $observacoes);
